...
INTRA_INTER_NUM_WARPS: list[int] = lasd_intra_inter_num_warps()
BLOCK_C_OPTIONS: list[int] = lasd_block_c_options()
BLOCK_D_OPTIONS: list[int]
BLOCK_E_OPTIONS: list[int]
BLOCK_D_OPTIONS, BLOCK_E_OPTIONS = lasd_block_de_options()
...
@triton.autotune(
    generate_configs(
        {
            "num_warps": [4, 8, 16, 32],
            "BLOCK_C": BLOCK_C_OPTIONS,
            "BLOCK_D": BLOCK_D_OPTIONS,
            "BLOCK_E": BLOCK_E_OPTIONS,
        }
    ),
    key=[
        "B",
...
def _lasd_parallel_state_parallel(
    ...
    k_trans_block_ptr = K + offset_qk + offset_block_qk + array_c[None, :] * H * D + (array_d + offset_block_d)[:, None]
    v_block_ptr = V + offset_vo + offset_block_vo + array_c[:, None] * H * E + (array_e + offset_block_e)[None, :]
    state_block_ptr = (
        STATES
        + offset_state
        + offset_block_state
    ...
    for i in range(NUM_BLOCK_C):
        ...
        if cnt < N:
            k_trans = tl.load(k_trans_block_ptr, mask=mask_c[None, :] & mask_d[:, None], other=0.0)
...
def _lasd_parallel_state_reduce(
    ...
    if REVERSE:
        ...
        last_block_decay_block_ptr = LOG_DECAY_CUMSUM + offset_ld + (NUM_BLOCK_N - 1) * BLOCK_N * H  # (N - C) * H
        ...
        last_block_decay_block_ptr = LOG_DECAY_CUMSUM + offset_ld + (N - 1) * H  # (BLOCK_N - 1) * H
    ...
    if USE_INITIAL_STATE:
        state_block_ptr = (
            STATE + offset_state + (offset_block_d + array_d[:, None]) * E + (offset_block_e + array_e[None, :])
        )
        # !!! important
...
def _lasd_parallel_state_parallel_reduce(
    ...
    for i in range(NUM_BLOCK_N):
        ...
        ld_sum_block_ptr = LOG_DECAY_CUMSUM + offset_ld + offset_block_ld + offset_ld_sum
        log_decay_sum = tl.load(ld_sum_block_ptr).to(tl.float32)
        ...
        ##### compute local state
        k_trans_block_ptr = (
            K + offset_qk + offset_block_qk + array_c[None, :] * H * D + (array_d + offset_block_d)[:, None]
        )
        v_block_ptr = V + offset_vo + offset_block_vo + array_c[:, None] * H * E + (array_e + offset_block_e)[None, :]
...
def _lasd_parallel_intra(
    ...
    # compute block ptr and mask
    q_block_ptr = Q + offset_qk + offset_block_qk + (offset_block_c + array_c[:, None]) * H * D + array_d[None, :]
    o_block_ptr = (
        O
        + offset_vo
        + offset_block_vo
    ...
    ldq_block_ptr = LOG_DECAY + offset_ld + offset_block_ld + (offset_block_c + array_c) * H
    ...
    for i in range(NUM_BLOCK_D):
        ...
        k_trans_block_ptr = (
            K + offset_qk + offset_block_qk + array_kv[None, :] * H * D + (i * BLOCK_D + array_d)[:, None]
        )
        v_block_ptr = V + offset_vo + offset_block_vo + array_kv[:, None] * H * E + (offset_block_e + array_e)[None, :]
        ldk_block_ptr = LOG_DECAY + offset_ld + offset_block_ld + array_kv * H

        for j in range(NUM_LOOP):
            mask_kv = (offset_block_n + array_kv) < N

            k_trans = tl.load(k_trans_block_ptr, mask=mask_kv[None, :] & mask_d[:, None], other=0.0)
...
def _lasd_parallel_inter(
    ...
    if TRANS:
        ...
        state_block_ptr = (
            STATES + offset_state + offset_block_state + array_d[:, None] + (offset_block_e + array_e)[None, :] * D
        )
    else:
        state_block_ptr = (
            STATES + offset_state + offset_block_state + array_d[:, None] * E + (offset_block_e + array_e)[None, :]
        )
    ...
    o = tl.load(o_block_ptr, mask=mask_c[:, None] & mask_e[None, :], other=0.0).to(tl.float32)
    for i in range(NUM_BLOCK_D):
        mask_d = (array_d + i * BLOCK_D) < D
        q = tl.load(q_block_ptr, mask=mask_c[:, None] & mask_d[None, :], other=0.0)
        state = tl.load(state_block_ptr, mask=mask_d[:, None] & mask_e[None, :], other=0.0).to(q.dtype)
...
@triton.autotune(
    generate_configs(
        {
            "num_warps": INTRA_INTER_NUM_WARPS,
        }
    ),
    key=[
        "B",
...
def _lasd_parallel_intra_inter(
    ...
    if REVERSE:
        stride = -1
        ldq_inter_block_ptr = LOG_DECAY_REVERSE + offset_ld + offset_block_ld + (offset_block_c + array_c) * H
    ...
    if REVERSE:
        ...
        mask_qk = (offset_block_c + array_c)[:, None] >= array_kv[None, :]
    mask_qk_ = mask_c[:, None] & mask_kv[None, :]
    mask_qk = mask_qk & mask_qk_
    ...
    if COMPUTE_DLD:
        ...
        x = tl.load(x_block_ptr, mask=mask_c[:, None] & mask_e[None, :], other=0.0).to(tl.float32)
        # N D -> N
        ...
        dld_block_ptr = (
            DLOG_DECAY + offset_dld + offset_block_dld + (offset_block_c + array_c) * H * NUM_BLOCK_E + off_block_e
        )
